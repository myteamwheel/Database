...
const BASE_COLS = {
  ...
  'opt.optimalMpg':{label:'Best MPG',type:'1',help:'WHAT: the minutes-per-game workload at which this player\u2019s projected on-court impact is highest. PLAIN: roughly how much he should play for the team to get the most out of him. FORMULA: TULIP projects impact at 12/16/20/24/28/32/36 MPG using comparable players at each workload; this is the band with the highest projection, counting only bands with enough comparables. Blank means fewer than two bands had support.'},
  'opt.minutesDelta':{label:'MPG +/-',type:'signed1',help:'WHAT: best MPG minus his current MPG. PLAIN: how many more (+) or fewer (-) minutes a game the evidence points to. FORMULA: opt.optimalMpg - current MPG. Positive means underused, negative means the projection peaks below what he already plays. This is an observational comparison against similar players, NOT a causal claim about what happens if his minutes change.'},
  'opt.projectedAtOptimal':{label:'Proj @ best',type:'signed2',help:'WHAT: projected on-court impact at the best-MPG workload. PLAIN: how good the model thinks he would be at that workload. FORMULA: the TULIP projected impact value of the winning role band.'},
  'opt.confidence':{label:'Best MPG conf',type:'2',help:'WHAT: how clearly the peak stands out. PLAIN: higher means the best workload is distinct, near 0 means the curve is flat and the peak is inside the noise. FORMULA: (max projected impact across supported bands - min) / width of the best band\u2019s confidence interval.'},
...
const PRESETS = {
  overall:['select','viewRank','rank','name','team','position','age','gp','mpg','grade','rateGrade','magnitudeGrade','tulip.leagueDelta','opt.optimalMpg','opt.minutesDelta','pts','reb','ast','stl','blk','ts','usg','pie','netRtg','custom.twoWayIndex','reliabilityWeight'],
...
const PRESET_LABELS = {overall:'Overall',tulip:'TULIP Role Expansion',workload:'Optimal Workload',nbaready:'NBA Readiness (G League)',per36:'Per 36 Minutes',per36nba:'Per 36 — NBA Equivalent (G League)',scoring:'Scoring',shooting:'Shooting',playmaking:'Playmaking',
